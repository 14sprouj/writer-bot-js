...
class Sprint {
	...
	async isDeclarationFinished() {
		...
		const results = await this._db.get_all_sql('SELECT * FROM writingpal_SprintUsers WHERE sprint = ? AND ending_wc = 0 AND (sprint_type IS NULL OR sprint_type != ?)', [
	...
	async join(user_id, start = 0, type = null, project_id = null) {
		...
		return this._db.insert('writingpal_SprintUsers', {
			'sprint': this.id,
			'user': user_id,
			'starting_wc': start,
			'current_wc': start,
	 ...
	 */
	async leave(user_id) {
		return this._db.delete('writingpal_SprintUsers', { 'sprint': this.id, 'user': user_id });
	}
	...
	async cancel() {
		...
		await this._db.delete('writingpal_SprintUsers', { 'sprint': this.id });
		await this._db.delete('writingpal_Sprints', { 'id': this.id });
	...
	async updateUser(user_id, start = false, current = false, ending = false, type = false, project_id = false) {
		...
		return this._'sprint_users', record;
 ...
 */
	 ...
	 */
	static async load(guildid, db) {
		const guildID = interaction.guild.id;
		const record = await db.get('sprints', { 'guild': guildID, 'completed': 0 });
		if (record) {
