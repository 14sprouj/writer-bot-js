...
class DB {
	 ...
	 */
	async connect() {
		this.connection = await this.connection.getConnection(function (err, connection) {
			if (err) {
				logger.error(`[CLUSTER ${client.cluster.id}] [DATABASE] Error connecting to database: ${err.stack}`);
				return;
			}
			logger.info(`[CLUSTER ${client.cluster.id}] [DATABASE] Connected to database`);
		});
	...
	async end() {
		...
		logger.info(`[CLUSTER ${client.cluster.id}] [DATABASE] Disconnected from database`);
	}
